...
df_masters_to_pmr_str = pd.read_csv(sr.fp_csv_experimental_design_pmr_strategies_by_master)
#dictionary that maps masters to strategies to run
dict_master_to_strat = sr.build_dict(df_masters_to_pmr_str[["master_id", "strategy_id"]])
...
############################################
#    GET AVAILABILITY OF FILES FOR COPY    #
############################################

#get all available strategies
all_strats_to_copy = os.listdir(sr.dir_ref_gams_input_data)
all_strats_to_copy = [int(x.replace("strategy_id-", "")) for x in all_strats_to_copy if ("strategy_id-" in x)]
all_strats_to_copy.sort()
#ensure 0 is there
if 0 not in all_strats_to_copy:
    print("NOTE: no baseline data for gams input found in '%s'. Check the directory and ensure that the directory for strategy 0 is present."%(sr.dir_ref_gams_input_data))
    exit()
#otherwise, build a dictionary of files
dict_strat_di_files = {}


for s in all_strats_to_copy:
    fp_check = os.path.join(sr.dir_ref_gams_input_data, "strategy_id-" + str(s))
    fis = os.listdir(fp_check)
    fis = [x for x in fis if ".csv" in x]
    #update the dictionary
    dict_strat_di_files.update({s: fis})


if sr.parallel_exec_gams_q:


    print("#"*30 + "\n###\n###\n###   NOTE: NEED TO ADAPT TO COPY OVER FILES FROM %s FOR EACH STRATEGY TO DATA INPUT\n###\n###\n" + "#"*30 + "\n"*3)
    ...
    for m in all_masters_run:
        ...
        ##  copy over files by strategy into gams data_input file
        
        #get the associated strategy
        strat = dict_master_to_strat[m]
        #set the path and get the available files
        dir_copy_dat = os.path.join(sr.dir_ref_gams_input_data, "strategy_id-" + str(strat))
        #copy over the files from input
        if not os.path.exists(dir_copy_dat):
            print("\nNOTE: path '%s' not found. Using strategy 0 for "%(dir_copy_dat))
            dir_copy_dat = os.path.join(sr.dir_ref_gams_input_data, "strategy_id-0")
            strat = 0
        #get available files
        fis = dict_strat_di_files[strat]

        print("\n"*2 + "#"*10 + "\n###\n###    Mapping master_id = %s to strategy_id %s\n###\n"%(m, strat) + "#"*10 + "\n"*2)
        
        #copy in
        for fi in fis:
            fp_src = os.path.join(dir_copy_dat, fi)
            if os.path.exists(fp_src):
                fp_tar = os.path.join(sr.dir_gams_input, fi)
                print("Copying '%s' to '%s'... "%(fp_src, fp_tar))
                shutil.copy(fp_src, fp_tar)
                print("Copy complete.")
            else:
                print("\n\nNOTE: path '%s' not found. The data may not be associated with the correct strategy.\n\n"%(fp_src))
